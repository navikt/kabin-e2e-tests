...
export class KabinPage {
  ...
  deleteRegistrering = async () => {
    ...
    if (registreringMatch !== null) {
      const [, regisreringId] = registreringMatch;

      if (typeof regisreringId === 'string') {
        await makeDirectApiRequest(this.page, 'kabin-api', `/registrering/${regisreringId}`, 'DELETE');

        console.info('Deleted registrering with id:', regisreringId);
